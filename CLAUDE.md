...
download.py (CLI)                    bot.py (Telegram bot)
     │                                    │
     ├─ detect_platform()                 ├─ _is_allowed() → auth check
     ├─ parse_{platform}_url()            ├─ _process_url() → download_media()
     ├─ _check_duplicate()                ├─ _send_files() → albums of 10
     ├─ _download_{platform}()            ├─ _serve_large_file() → nginx link
     ├─ _ensure_h264()                    └─ _handle_channel_download()
     ├─ build_filenames()                      → run_in_executor()
     └─ _print_summary()
```
...
**Video post-processing**: `_ensure_h264()` checks codec via ffprobe and re-encodes VP9 to H.264 using ffmpeg. Original file deleted only after successful encode.

## Bot Architecture (bot.py)

**Access control**: Restricted to allowlisted users. Two verification methods:
- Username match: `ALLOWED_USERS` set (checked on every request)
- Phone verification: `/auth` command → share contact → checked against `ALLOWED_PHONES`
- Verified user IDs cached in `_verified_user_ids` (in-memory, lost on restart)
...
- **Cookie files**: `x_cookies.txt`, `www.instagram.com_cookies.txt` — Netscape format, gitignored via `*cookies*.txt`
- **Telegram auth**: telethon session file (`telegram.session`). API credentials via env vars with defaults.
- **URL parsing**: Returns `tuple[str, str]` for (username, id). Telegram returns `tuple[str, str | None]` where None = full channel.
...
- **Files on server** (not in git): `x_cookies.txt`, `www.instagram.com_cookies.txt`, `telegram.session`
